...
public class PathingMain extends PApplet
{
   ...
   private boolean moveOnce(Point pos, GridValues[][] grid, List<Point> path)
   {
       ...
       {
           //check if my neighbor is the goal
           ...
           grid[pos.y][pos.x] = GridValues.SEARCHED;
           found = moveOnce(rightN,grid,path)||moveOnce(leftN,grid,path)||moveOnce(downN,grid,path)||moveOnce(upN,grid,path);
           if (found){path.add(0,pos);}
       }
       return found;
   }
